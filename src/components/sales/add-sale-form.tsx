...
const formSchema = z.object({
  ...
  mobileNumber: z.string().regex(/^\+91 \d{5} \d{5}$/, { message: "Invalid mobile number format." }),
...
export default function AddSaleForm({ setOpen }: AddSaleFormProps) {
  const { toast } = useToast()
  const { staff, therapies, rooms, addSale } = useData();
