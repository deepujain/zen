...
class DatabaseService {
  ...
  public bulkUpdateTherapyType(fromTherapyTypes: string[], toTherapyType: string): Promise<void> {
    return this.promisify<void>((callback) => {
      const placeholders = fromTherapyTypes.map(() => '?').join(', ');
      this.db.run(
        `UPDATE sales SET therapyType = ? WHERE therapyType IN (${placeholders})`,
        [toTherapyType, ...fromTherapyTypes],
        callback
      );
    });
  }
  ...
  public getAllExpenses(): Promise<Expense[]> {
    return this.promisify<Expense[]>((callback) => {
      this.db.all('SELECT * FROM expenses ORDER BY date DESC', (err, rows: Expense[]) => {
        if (err) {
  ...
  // Bulk delete methods for migration
  public deleteAllStaff(): Promise<void> {
    return this.promisify<void>((callback) => {
      this.db.run('DELETE FROM staff', callback);
    });
  }

  public deleteAllRooms(): Promise<void> {
    return this.promisify<void>((callback) => {
      this.db.run('DELETE FROM rooms', callback);
    });
  }

  public deleteAllTherapies(): Promise<void> {
    return this.promisify<void>((callback) => {
      this.db.run('DELETE FROM therapies', callback);
    });
  }

  public deleteAllSales(): Promise<void> {
    return this.promisify<void>((callback) => {
      this.db.run('DELETE FROM sales', callback);
    });
  }

  public deleteAllExpenses(): Promise<void> {
    return this.promisify<void>((callback) => {
      this.db.run('DELETE FROM expenses', callback);
    });
  }

  public deleteAllAttendance(): Promise<void> {
    return this.promisify<void>((callback) => {
      this.db.run('DELETE FROM attendance', callback);
    });
  }
}
